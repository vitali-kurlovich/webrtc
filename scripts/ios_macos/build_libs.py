...
"""WebRTC iOS/MacOS XCFramework build script.
...
ENABLED_ARCHS = ["device:arm64", "simulator:arm64", "macos:arm64", "arm64"]

DEFAULT_ARCHS = ["device:arm64", "simulator:arm64", "macos:arm64"]

MINIMUM_DEPLOYMENT_TARGET = {"device": "16.0", "simulator": "16.0", "macos": "14.0"}
...
def _ParseArgs():
    ...
    parser.add_argument(
        ...
        help='The build config. Can be "debug" or "release". Defaults to "release".',
    )
    parser.add_argument(
        "--arch",
        nargs="+",
    ...
    parser.add_argument(
        "--use-remoteexec", action="store_true", default=False, help="Use RBE to build."
    )
    parser.add_argument(
        "--deployment-target",
        default=MINIMUM_DEPLOYMENT_TARGET["device"],
...
def BuildWebRTC(
    ...
    if target_environment == "macos":
        gn_args.append('target_os="mac"')
        gn_args.append('mac_deployment_target="%s"' % deployment_target)

    else:
        gn_args.append('target_os="ios"')
        gn_args.append("ios_enable_code_signing=false")
        gn_args.append('ios_deployment_target="%s"' % deployment_target)
    ...
    # Codecs
    gn_args.append("rtc_libvpx_build_vp9=true")
    gn_args.append("rtc_system_openh264=true")
...
def main():
    ...
    gn_mac_target_name = "mac_framework_objc"
    ...
    for environment, archs in list(architectures.items()):
        deployment_target = MINIMUM_DEPLOYMENT_TARGET[
            environment
        ]  # _VersionMax(MINIMUM_DEPLOYMENT_TARGET[environment])
        ...
        for arch in archs:
            ...
            if environment == "macos":
                ...
                mac_all_lib_paths.extend(lib_paths)
            else:
                BuildWebRTC(
                    lib_path,
                    environment,
        ...
        shutil.rmtree(
            os.path.join(framework_path, SDK_FRAMEWORK_NAME), ignore_errors=True
        )
        shutil.copytree(
        ...
        if os.path.islink(out_dylib_path):
            out_dylib_path = os.path.join(
                os.path.dirname(out_dylib_path), os.readlink(out_dylib_path)
            )
        try:
        ...
        if os.path.isdir(lib_dsym_dir_path):
            shutil.rmtree(
                os.path.join(framework_path, SDK_DSYM_NAME), ignore_errors=True
            )
            shutil.copytree(
            ...
            dsym_path = os.path.join(
                SDK_DSYM_NAME, "Contents", "Resources", "DWARF", "WebRTC"
            )
            ...
            resources_dir = os.path.join(
                framework_path, SDK_FRAMEWORK_NAME, "Resources"
            )
            ...
            cmd = [
                "PlistBuddy",
                "-c",
                "Set :CFBundleVersion " + version_number,
                infoplist_path,
            ]
            _RunCommand(cmd)
    ...
    logging.info("gn_target_full_name: %s", gn_target_full_name)
    logging.info("all_lib_paths: %s", all_lib_paths)

    builder = LicenseBuilder(all_lib_paths, [gn_target_full_name])

    builder.generate_license_text(os.path.join(args.output_dir, SDK_XCFRAMEWORK_NAME))
